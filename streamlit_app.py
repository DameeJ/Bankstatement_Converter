...
if uploaded_file is not None:
    ...
    if st.button("Convert to Excel", type="primary"):
        ...
        try:
            ...
            if stats["split_mode"] == "year":
                col3.metric("Years covered", stats["n_years"])
            else:
                col3.metric("Months covered", stats["n_months"])

            if stats["split_mode"] == "year":
                st.caption(f"Statement spans {stats['n_years']} years — split into one sheet per year.")
            elif stats["split_mode"] == "month":
                st.caption(f"Large single-year statement — split into one sheet per month.")
