...
def parse_date(value):
    value = value.strip()
    value = re.sub(r"\s*-\s*", "-", value)  # "30-May- 2024" -> "30-May-2024"
...
def export_to_excel(raw_export_df, cleaned_df, dropped_df, schema, desc_col, output_path, split_by_month):
    display_cols = [c["display"] for c in schema]
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        if split_by_month:
            for month, group in cleaned_df.groupby("_Month", sort=False):
                sheet_name = str(month)[:31]
                month_export = group[display_cols]
                month_export.to_excel(writer, sheet_name=sheet_name, index=False)
                autofit_columns(writer.sheets[sheet_name], month_export)
...
def convert(pdf_path, output_path, progress=print):
    ...
    split_by_month = len(cleaned_df) > MONTH_SPLIT_THRESHOLD
    n_months = cleaned_df["_Month"].nunique() if len(cleaned_df) else 0
    progress(f"[6/6] Exporting ({'split into ' + str(n_months) + ' monthly sheets' if split_by_month else 'single sheet, under threshold'})...")
    export_to_excel(raw_export_df, cleaned_df, dropped_df, schema, desc_col, output_path, split_by_month)
